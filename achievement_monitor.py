...
class AchievementMonitor:
    ...
    async def get_player_achievements(
        self, api_key: str, group_id: str, steamid: str, appid: int
    ) -> set[str] | None:
        """
                ...
                try:
                    async with httpx.AsyncClient(timeout=15) as client:
                        ...
                        if response.status_code == 200:
                            ...
                            if (
                                "playerstats" in data
                                and "achievements" in data["playerstats"]
                            ):
                                ...
                                # 检查是否有描述字段且不全为空
                                has_desc = any(
                                    ach.get("description") for ach in achievements
                                )
                                if has_desc:
                                    return unlocked
                                # 否则继续尝试下一个语言
                            ...
                            print(
                                f"无权限获取玩家 {steamid} 的游戏 {appid} 成就数据 (隐私设置)"
                            )
                            return None
                        else:
                            print(
                                f"获取成就数据失败: HTTP {response.status_code} (第{attempt + 1}次, lang={lang})"
                            )
                except Exception as e:
            ...
            try:
                async with httpx.AsyncClient(timeout=15) as client:
                    ...
                    if resp.status_code == 400:
                        print(
                            f"获取成就schema失败: HTTP 400，通常为appid错误或该游戏无成就，appid={appid}，尝试降级用GetPlayerAchievements"
                        )
                        ...
                        for lang2 in lang_list:
                            ...
                            if resp2.status_code == 200:
                                try:
                                    ...
                                    print(
                                        f"降级解析GetPlayerAchievements json失败: {e} resp.text={resp2.text[:200]}"
                                    )
                            else:
                                print(
                                    f"降级GetPlayerAchievements失败: HTTP {resp2.status_code}"
                                )
                        return details
                    ...
                    try:
                        ...
                        print(
                            f"解析成就schema json失败: {e} resp.text={resp.text[:200]}"
                        )
                        continue
                    achievements = {}
                    if "game" in schema and "availableGameStats" in schema["game"]:
                        for ach in schema["game"]["availableGameStats"].get(
                            "achievements", []
                        ):
                    ...
                    if resp2.status_code != 200:
                        print(
                            f"获取成就解锁率失败: HTTP {resp2.status_code} url={url_stats}"
                        )
                        percents = {}
                    else:
                        try:
                            ...
                            print(
                                f"解析成就解锁率json失败: {e} resp.text={resp2.text[:200]}"
                            )
                            stats = {}
                        percents = {}
                        if (
                            "achievementpercentages" in stats
                            and "achievements" in stats["achievementpercentages"]
                        ):
    ...
    async def check_new_achievements(
        ...
        current_achievements = await self.get_player_achievements(
            api_key, group_id, steamid, appid
        )
    ...
    def render_achievement_message(
        self, achievement_details: dict, new_achievements: set, player_name: str = ""
    ) -> str:
        lines = []
        trophy = "🏆"
    ...
    async def render_achievement_image(
        ...
        try:
            ...
            font_title = font_game = font_name = font_desc = font_percent = (
                font_game_small
        ...
        if unlocked_set is None:
            ...
            if steamid is not None and appid is not None:
                unlocked_set = (
                    await self.get_player_achievements(
                        os.environ.get("STEAM_API_KEY", ""), steamid, appid
                    )
                    or set()
                )
        ...
        progress_percent = (
            int(unlocked_achievements / total_achievements * 100)
        ...
        if not game_name:
            game_name = next(
                (
                    d.get("game_name")
                    for d in achievement_details.values()
                    if d and d.get("game_name")
                ),
                "",
            )
        if not game_name:
        ...
        title_game_gap = 8
        header_h = (
            title_h + title_game_gap + game_h + progress_bar_h + progress_bar_margin * 3
        )
        ...
        for apiname in new_achievements:
            ...
            # 估算卡片高度
            card_h = max(
                icon_size + 24, len(name_lines) * 22 + len(desc_lines) * 18 + 60
            )
        ...
        # 玩家名解锁新成就（大字）
        draw.text(
            (padding_h, padding_v), title_text, fill=(255, 255, 255), font=font_title
        )
        # 游戏名（更小更淡，换行在下方，增加间距）
        draw.text(
        ...
        # 当前时间（右上角，更小更淡）
        draw.text(
            (width - padding_h - time_w, padding_v),
            now_str,
            fill=(168, 168, 168),
            font=font_time,
        )
        ...
        # 文字
        progress_text = (
            f"{unlocked_achievements}/{total_achievements} ({progress_percent}%)"
        )
        ...
        draw.text(
            (bar_x + bar_w - progress_text_w - 6, bar_y - 2),
            progress_text,
            fill=(142, 207, 255),
            font=font_percent,
        )
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for apiname in new_achievements:
                ...
                mask_draw.rounded_rectangle(
                    (0, 0, card_w, card_hh), radius=card_radius, fill=255
                )
                    ...
                    try:
                        async with session.get(icon_url) as response:
                            if response.status == 200:
                                ...
                                icon_img = Image.open(io.BytesIO(icon_data)).convert(
                                    "RGBA"
                                )
                                ...
                                ImageDraw.Draw(mask_icon).rounded_rectangle(
                                    (0, 0, icon_size, icon_size), 12, fill=255
                                )
                ...
                if icon_img:
                    if percent_num < 10:
                        ...
                        img.alpha_composite(
                            icon_canvas, (icon_x - glow_size, icon_y - glow_size)
                        )
                    else:
                ...
                percent_label_bbox = draw.textbbox(
                    (0, 0), percent_label, font=percent_label_font
                )
                ...
                if percent_num < 10:
                    ...
                    for r in range(glow_radius, 0, -4):
                        draw.text(
                            ...
                            font=percent_label_font,
                        )
                    # value发光
                    percent_value_bbox = draw.textbbox(
                        (0, 0), percent_value_str, font=percent_value_font
                    )
