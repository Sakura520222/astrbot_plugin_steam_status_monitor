...
class SteamStatusMonitorV2(Star):
    ...
    async def test_game_start_render(self, event: AstrMessageEvent, steamid: str, gameid: int):
        ...
        try:
            ...
            # 获取英文名
            url_en = f"https://store.steampowered.com/api/appdetails?appids={gameid}&l=en"
            en_game_name = None
            try:
                async with httpx.AsyncClient(timeout=10) as client:
                    resp_en = await client.get(url_en)
                ...
                en_game_name = None
            logger.info(f"[测试开始游戏渲染] steamid={steamid} gameid={gameid} player_name={player_name} avatar_url={avatar_url} game_name={zh_game_name} en_game_name={en_game_name}")
            ...
            img_bytes = await render_game_start(
                self.data_dir, steamid, player_name, avatar_url, gameid, zh_game_name, api_key=self.API_KEY, superpower=superpower, sgdb_api_key=self.SGDB_API_KEY, font_path=font_path, sgdb_game_name=en_game_name
            )
